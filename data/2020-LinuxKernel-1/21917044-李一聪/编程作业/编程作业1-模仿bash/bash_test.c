...
int read_command(char d_name[],struct ps_info *p1); /* 读取进程信息 */
void output(struct ps_info *head); /* 打印进程信息 */
int isnum(char *); /* 判断是否为数字*/
void init_cmd(); /* 由进程uid得到进程的所有者user */
struct ps_info *parse_token(char dir[]); /* 遍历指定目录建立进程信息链表*/
...
int main() {
	...
	/* 运行外部程序 */
	    ...
	    if (pid == 0) {
		    ...
		    CPU_SET(atoi(args[2])-1, &mask); /* 将指定的cpu加入到集合 */
...
mps *parse_token(char dir[]) /* 遍历指定目录，并建立进程信息链表*/
{
    DIR *dir_ptr;
    mps *head,*p1,*p2;
...
int read_command(char d_name[],struct ps_info *p1) /* 读取进程相关信息 */
{
    FILE *fd;
    char dir[20];
    struct stat infobuf;
...
void init_cmd(uid_t uid, struct ps_info *p1)  /* 由进程uid得到进程的所有者user */
{
