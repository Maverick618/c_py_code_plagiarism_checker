...
post_seqs = [[] for x in range(len(hw_paths))]
...
for i in range(len(hw_paths)):
    ...
    if not pre_seqs[i]:
        ...
        pre_seqs[i], post_seqs[i] = hw.get_ast_seq()
    while j < len(hw_paths):
        p_2 = hw_paths[j]
        if not pre_seqs[j]:
            hw = Homework(p_2, code_type)
            pre_seqs[j], post_seqs[j] = hw.get_ast_seq()
        similarity_matrix[i][j] = get_similarity(pre_seqs[i], post_seqs[i], pre_seqs[j], post_seqs[j])
